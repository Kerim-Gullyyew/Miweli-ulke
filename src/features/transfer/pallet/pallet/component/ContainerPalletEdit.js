...
const ContainerPalletEdit = () => {
  ...
  const [price, setPrice] = useState(containerPallet?.product?.price);
  const views = useSelector((state) => state?.views);
  const [product, setProduct] = useState(containerPallet?.product);
  ...
  const onContainerPalletEditClicked = (e) => {
    ...
    dispatch(editContainerPallet({ id, token, json }))
      ...
      .then((res) => {
        ...
        console.log("editContPallres", jsontrans);
        dispatch(
                ...
                <div className="flex flex-wrap justify-center">
                  {stock.data.map((stock, key) => (
                    <div
                      ...
                      className=" w-[150px] h-[150px] border rounded-lg justify-between flex cursor-pointer flex-col m-2"
                    >
