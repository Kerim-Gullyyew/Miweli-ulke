...
import { editmovedetail, getMoveDetail } from "../moveDetailSlice";

const MoveEdit = ({ moveDetail1 }) => {
  ...
  const onBatchContainerEditClicked = (e) => {
    ...
    const json = {
      out_cell_id: selectedCell ? selectedCell.id : moveDetail.out_cell.id,
      in_cell_id: selectedCell2 ? selectedCell2.id : moveDetail.in_cell.id,
      user: moveDetail.user.id,
    };

 
    let isCancelled = false;
    dispatch(editmovedetail({ id: moveDetail.id, token, json }))
      .unwrap()
      .then((res) => {
        dispatch(getMoveDetail({ token, id: moveDetail.id }));
        dispatch(stockView(""));
        dispatch(other(""));
        SetError("");
        setOpen(false);
                                              ...
                                              <div className="mx-auto">
                                                {col.cells.map((cell, key) => (
                                                  <Fragment key={key}>
                                                    {/* {cell.product !== null ? ( */}
                                                      <div
                                                        onClick={() => cell.is_full === true ? setselectedCell(cell) : setselectedCell("")}
                                              ...
                                              <div className="mx-auto">
                                                {col.cells.map((cell, key) => (
                                                  <Fragment key={key}>
                                                    {/* {cell.product === null ? ( */}
                                                      <div
                                                        onClick={() => cell.is_full === false ? setselectedCell2(cell) : setselectedCell2("")}
