...
import { editreducedetail } from "../reduceDetailSlice";
import { getreduce } from "../reduceSlice";

const ReduceEdit = ({ reduceDetail1 }) => {
  ...
  const [reduceDetail, setreduceDetail] = useState("");
  ...
  useEffect(() => {
    setId(reduceDetail1.id);
    setreduceDetail(reduceDetail1);
    setPrice(reduceDetail1.price);
    setAmount(reduceDetail1.amount);
    setOldImage(reduceDetail1.image);
    setReason(reduceDetail1.reason);
    setDescription(reduceDetail1.description);
  }, [reduceDetail1]);
  ...
  const onReduceEditClicked = (e) => {
    ...
    formData.append(
      "image",
      selectedImage === undefined ? oldImage : selectedImage
    );
    formData.append("description", description);
    formData.append(
      "pallet_id",
      selectedCell ? selectedCell.pallet.id : reduceDetail.pallet.id
    );
    formData.append(
      "pallet_attribute_id",
      selected ? selected.attr_id : reduceDetail.attribute.id
    );
    formData.append("user", user);

    dispatch(editreducedetail({ token, id, formData }))
      .unwrap()
      .then((res) => {
                          ...
                          <div className="grid grid-cols-10 items-center">
                            ...
                            <div className="grid col-span-5 w-32 items-center h-32">
                              {previewImage ? (
                                ...
                                <img src={backendUrl + oldImage} alt="Img" />
                              )}
                            </div>
                          </div>
