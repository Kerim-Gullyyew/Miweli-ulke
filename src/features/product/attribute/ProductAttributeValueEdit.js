...
import { productAttributeValueEdit } from "../product/productDetailSlice";

const ProductAttributeValueEdit = ({ attr_id, id, value }) => {
  const [open, setOpen] = useState(false);
  const cancelButtonRef = useRef(null);
  const dispatch = useDispatch();
  const [productAttributeValue, setProductAttributeValue] = useState(value);
  const [error, SetError] = useState("");
  const product_id = useSelector((state) => state.productDetail.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(productAttributeValueEdit({ token, id, json }))
        .unwrap()
        .then((res) => {
          setOpen(false);
          SetError("");
...
export default ProductAttributeValueEdit;
