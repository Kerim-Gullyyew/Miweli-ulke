...
import { ProductUnitNewContext } from "../ProductContext";
import { editProductUnit } from "./productUnitSlice";

const ProductUnitEdit = ({ unit_id }) => {
  ...
  useEffect(() => {
    function b(unit_id) {
      return productUnit.data.filter((item) => {
        return item.id === unit_id;
      });
    }
    setProductUnitName(b(Number(unit_id))[0]?.title);
  }, [unit_id, productUnit.data]);

  const onSetproductUnitNameChanged = (e) => {
    setProductUnitName(e.target.value);
  };

  const ProductUnitCreate = (e) => {
    e.preventDefault();
    ...
    dispatch(editProductUnit({ token, id: unit_id, json }))
      .unwrap()
      .then((res) => {
  ...
  return (
    <>
      <FaPen
        ...
        size={20}
        className="text-blue-700 mr-3 cursor-pointer"
      />
                    ...
                    <div className="">
                      <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        ...
                        <div className="mt-2 grid grid-cols-1 space-y-3">
                          ...
                          <label htmlFor="productUnitName">
                            {ProductUnitNewContext.name}
                          </label>

                          <div className="flex items-center border-b border-blue-500 py-2">
                            <input
                              value={productUnitName}
                              onChange={onSetproductUnitNameChanged}
