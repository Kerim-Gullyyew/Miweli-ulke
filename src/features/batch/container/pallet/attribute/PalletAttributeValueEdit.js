...
import { editContainerDetailAttributeValue } from "../../containerDetailSlice";
import { editbatchDetailPalletAttributeValue } from "../../../batch/batchDetailSlice";
import { palletAttributeValueEdit } from "../containerPalletSlice";

const PalletAttributeValueEdit = ({ id, attr_id, value }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const json = {
      ...
      pallet_id: pallet_id,
      pallet_attr_id: attr_id,
    };
    let isCancelled = false;

    try {
      await dispatch(palletAttributeValueEdit({ token, id, json }))
        .unwrap()
        .then((res) => {
          dispatch(editContainerDetailAttributeValue({pallet_id, data: res.data}));
        dispatch(editbatchDetailPalletAttributeValue({cont_id, pallet_id, data: res.data}));
          setOpen(false);
          SetError("");
        });
    } catch (err) {
...
export default PalletAttributeValueEdit;
