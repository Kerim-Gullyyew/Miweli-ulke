...
import { containerAttributeValueDelete } from "../../containerDetailSlice";
import { batchContainerAttributeValueDelete } from "../../../batch/batchDetailSlice";

const ContainerAttributeValueDelete = ({ attr_id }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await dispatch(containerAttributeValueDelete({ token, id: attr_id }))
        .unwrap()
        .then((res) =>
          dispatch(batchContainerAttributeValueDelete({ cont_id, attr_id })));
          setOpen(false);
          SetError("");
    } catch (err) {
      SetError(err);
...
export default ContainerAttributeValueDelete;
